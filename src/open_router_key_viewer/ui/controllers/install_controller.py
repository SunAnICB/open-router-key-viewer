...
from open_router_key_viewer.core.install_runtime import build_install_coordinator
from open_router_key_viewer.i18n import tr
from open_router_key_viewer.state import InstallCardViewModel, TextSpec
...
class AboutInstallController:
    ...
    def __init__(self, host: QWidget, install_card: InstallCard, on_install_state_changed: Callable[[], None]) -> None:
        ...
        self._install_coordinator = build_install_coordinator()
    ...
    def install_info(self):
        return self._install_coordinator.install_info

    def refresh_install_info(self) -> None:
        self._install_coordinator.refresh()
        self.show_state()

    def show_state(self) -> None:
        self._apply_install_card_state(self._install_coordinator.current_view_model())
    ...
    def install_or_upgrade(self) -> None:
        box = MessageBox(
            ...
            _tr("将当前二进制安装到固定目录：{root}\n同时会写入 ~/.local/bin 启动入口、桌面启动器和图标。是否继续？").format(
                root=str(self._install_coordinator.install_root)
            ),
            self.host.window(),
        )
        ...
        if not box.exec():
            return
        result = self._install_coordinator.install_or_upgrade(APP_DISPLAY_NAME)
        if not result.ok:
            show_error_bar(self.host, _tr("安装失败"), result.message)
            return

        InfoBar.success(
            title=_tr("安装完成"),
            content=_tr("已安装到 {root}").format(root=str(self._install_coordinator.install_root)),
            parent=self.host,
    ...
    def remove_installation(self) -> None:
        if not self._install_coordinator.install_info.is_installed:
            return
        box = MessageBox(
            _tr("移除安装"),
        ...
        if not box.exec():
            return
        result = self._install_coordinator.remove_installation()
        if not result.ok:
            show_error_bar(self.host, _tr("移除失败"), result.message)
            return
    ...
    def open_install_directory(self) -> None:
        install_root = self._install_coordinator.install_info.install_root
